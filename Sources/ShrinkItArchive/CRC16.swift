 ...
 */
public class CRC16 : java.util.zip.Checksum {
  public typealias Checksum = CRC16
  ...
  private var value : Int = 0;
	...
	/*static { // initialize of static lookup table with values directly instead of
		for (int i = 0; i < 256; i++) {
			int crc = i << 8;
			for (int j = 0; j < 8; j++) {
	 ...
	 */
  private func update(_ crcInput : Int, _ bytes : [UInt8], _ off : Int, _ len : Int) -> Int {
    var crc = crcInput
    for i in off..<(off + len) {
			let b = (bytes[i] & 0xff);
      crc = (CRC16.table[((crc >> 8) & 0xff) ^ Int(b)] ^ (crc << 8)) & 0xffff;
		}
		return crc;
	}

	public static func getTable() -> [Int] {
		return table;
	}
  ...
  public func update(_ b : Int) {
	 ...
	 */
  public func update(_ b : [UInt8]) {
		value = update(value, b, 0, b.count);
	}
	 ...
	 * @param len number of bytes to process
	 */
  public func update(_ b : [UInt8], _ off : Int, _ len : Int) {
